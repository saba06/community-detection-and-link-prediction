...
def complexity_of_bfs(V, E, K):
    ...
    every vertex & edge is explored in worst case
    """
    ###TODO
    return abs(V)+abs(E)
...
def make_training_graph(graph, test_node, n):
    ...
    neighbors = sorted(graph.neighbors(test_node))[0:n]
    edges_to_remove=[((test_node,node) for node in neighbors)]
...
def jaccard(graph, node, k):
    ...
    for n in set(graph.nodes())-neighbors:
    ...
    for score in scores:
        if recommend_list!=[]:
             previous=recommend_list[len(recommend_list)-1]
             if previous[1]==score[1]:
                 continue
             else:
                 recommend_list.append(score)
        else :
            recommend_list.append(score)
    return recommend_list[1:k+1]
...
def path_score(graph, root, k, beta):
    ...
    for n in set(graph.nodes())-neighbors:
        scores.append(((root,n), 1. * beta**node2distances[n]*node2num_paths[n]))
...
def evaluate(predicted_edges, graph):
    ...
    ###TODO
    # sorting edge names
    predicted_edges = [(j,k) if j<k else (k,j) for j,k in predicted_edges]
    edges = [(j,k) if j<k else (k,j) for j,k in graph.edges()]
    return float(len(set(predicted_edges) & set(edges)))/float(len(predicted_edges))
